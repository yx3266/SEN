...
Save as entity_cluster.npz in the inverse_search path.
...
parser = argparse.ArgumentParser(description='Perform semantic clustering for textual evidence of entity type')
...
for i in range(start_idx, len(dataset)):
    ...
    if not os.path.isfile(os.path.join(inverse_dir, "entities_features2.npz")):
        print("********** entities not exist **********")
        continue
    else:
        try:
            # load features
            entity_feature = np.load(os.path.join(inverse_dir, "entities_features2.npz"))["arr_0"]
            query_feature = np.load(os.path.join(inverse_dir, "qCap_sbert_features.npz"))["arr_0"]
            all_feature = np.concatenate((entity_feature, query_feature), axis=0)
            # clustering
            ...
            np.savez(os.path.join(inverse_dir, "entity_cluster.npz"), sim_class=sim_class, max_class=max_class,
