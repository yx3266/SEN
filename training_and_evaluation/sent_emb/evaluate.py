'''
The file used for evaluating the model in the repo.
'''
...
parser.add_argument('--shared_cap_proj', action='store_true',
                    help='whether to use the same project for the caption in the caption and entities memories')
parser.add_argument('--shared_cap_ent_mem', action='store_true',
...
parser.add_argument('--filter_dup', action='store_true',
...
parser.add_argument('--img_cluster', action='store_true',
                    help='whether to use sen for visual evidence in general feature')
parser.add_argument('--places_cluster', action='store_true',
                    help='whether to use sen for visual evidence in places feature')
parser.add_argument('--cap_cluster', action='store_true',
                    help='whether to use sen for textual evidence of sentence type')
...
train_dataset = dataset.NewsContextDatasetEmbs(data_items_train, args.visual_news_root, args.queries_dataset_root,
                                               args.news_clip_root, \
                                               domain_to_idx_dict, 'train', sent_emb_dim,
                                               load_clip_for_queries=True if args.consistency == 'clip' and not args.use_clip_for_all else False, \
                                               load_clip_for_all=args.use_clip_for_all,
                                               labels_overlap=args.labels_overlap, \
                                               ner_ent=args.ner_ent, ner_cap=args.ner_cap,
                                               filter_duplicates=args.filter_dup, \
                                               img_cluster=args.img_cluster, places_cluster=args.places_cluster,
                                               cap_cluster=args.cap_cluster, ent_cluster=args.ent_cluster)

val_dataset = dataset.NewsContextDatasetEmbs(data_items_val, args.visual_news_root, args.queries_dataset_root,
                                             args.news_clip_root, \
                                             domain_to_idx_dict, 'val', sent_emb_dim,
                                             load_clip_for_queries=True if args.consistency == 'clip' and not args.use_clip_for_all else False, \
                                             load_clip_for_all=args.use_clip_for_all,
                                             labels_overlap=args.labels_overlap, \
                                             ner_ent=args.ner_ent, ner_cap=args.ner_cap,
...
test_dataset = dataset.NewsContextDatasetEmbs(data_items_test, args.visual_news_root, args.queries_dataset_root,
                                              args.news_clip_root, \
                                              domain_to_idx_dict, 'test', sent_emb_dim,
                                              load_clip_for_queries=True if args.consistency == 'clip' and not args.use_clip_for_all else False, \
                                              load_clip_for_all=args.use_clip_for_all,
                                              labels_overlap=args.labels_overlap, \
                                              ner_ent=args.ner_ent, ner_cap=args.ner_cap,
...
train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=collate_padding,
                              num_workers=args.num_workers, pin_memory=True)
val_dataloader = DataLoader(val_dataset, batch_size=args.eval_batch_size, shuffle=False, collate_fn=collate_padding,
                            num_workers=args.num_workers, pin_memory=True)
test_dataloader = DataLoader(test_dataset, batch_size=args.eval_batch_size, shuffle=False, collate_fn=collate_padding,
                             num_workers=args.num_workers, pin_memory=True)
...
model_settings = {'use_img_memory': args.use_img_memory, 'use_cap_memory': args.use_cap_memory,
                  'use_ent_memory': args.use_ent_memory, \
                  'use_src': args.use_src, 'use_places_memory': args.use_places_memory,
                  'domains_num': len(domain_to_idx_dict), 'domains_dim': args.domains_dim, \
                  'img_dim_in': img_features_dim, 'img_dim_out': args.mem_img_dim_out, 'places_dim_in': places_mem_dim,
                  'places_dim_out': args.mem_places_dim_out, \
                  'ent_dim_in': sent_emb_dim, 'ent_dim_out': args.mem_ent_dim_out, 'sent_emb_dim_in': sent_emb_dim,
                  ...
                  'fusion': args.fusion, 'pdrop': args.pdrop, 'inp_pdrop': args.inp_pdrop, 'pdrop_mem': args.pdrop_mem,
                  'emb_pdrop': args.emb_pdrop, \
                  'nlayers': args.nlayers, 'fc_dims': args.fc_dims, \
                  'img_mem_hops': args.img_mem_hops, 'cap_mem_hops': args.cap_mem_hops,
                  'ent_mem_hops': args.ent_mem_hops, 'places_mem_hops': args.places_mem_hops, \
                  'use_clip_for_all': args.use_clip_for_all, \
                  'ner_cap': True if args.ner_cap != '' else False, 'ner_ent': True if args.ner_ent != '' else False,
                  'labels_overlap': args.labels_overlap, \
                  'img_cluster': args.img_cluster, 'places_cluster': args.places_cluster,
                  'cap_cluster': args.cap_cluster, 'ent_cluster': args.ent_cluster}
...
if args.lr_sched == 'cycle':
    ...
    scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, args.lr, args.lr_max, step_size_up=steps_up,
                                                  step_size_down=steps_down, cycle_momentum=False)
else:
    scheduler = None
...
def evaluate_save(loader):
    ...
    return avg_loss, acc, acc_falsified, acc_match, result, (correct, correct_falsified, correct_match), (
    total_num, total_num_falsified, total_num_match)
...
if os.path.isfile(os.path.join(args.exp_folder, 'best_model_loss.pth.tar')):
    ...
    # Run on test data.
    test_loss, test_acc, test_acc_false, test_acc_match, test_result, test_correct_fm, test_total_fm = evaluate_save(
        test_dataloader)
    print('=' * 89)
